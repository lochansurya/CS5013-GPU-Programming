...
//  could be varied for varying recall/QPS in plots (apart from L, L is the typically varied for
//  plots)
...
// for a parent, then iteration will occur. There is one initial iteratoion where Medoid is added
// (outside do-while)
#define MAX_PARENTS_PERQUERY \
    (4 * L + 20)  // Needs to be set with expereince. set it to (2*L) if in doubt

// length of each entry in INDEX file 128 bytes (FP) + 4 bytes (degree) + 256 bytes (AdjList for
// R=64) There are N such entries
...
void parANN(int argc, char** argv) {
    ...
    queriesFP = (datatype_t*)malloc(sizeof(datatype_t) * numQueries *
                                    D);  // full floating point coordinates of queries
    ...
    // Note : R+1 is needed because MEDOID is added as additional neighbour in very first neighbour
    // fetching
    ...
    gpuErrchk(
        cudaMallocHost(&FPSetCoordsList, (MAX_PARENTS_PERQUERY * numQueries) * FPSetCoords_size));
    ...
    // Allocations on GPU
    // gpuErrchk(cudaMalloc(&d_compressedVectors, sizeof(uint8_t) * N * CHUNKS)); 	//100M*100
    // ~10GB
    ...
    gpuErrchk(
        cudaMalloc(&d_FPSetCoordsList, (MAX_PARENTS_PERQUERY * numQueries) *
                                           FPSetCoords_size));  // Dim: [numIterations * numQueries]
    gpuErrchk(cudaMalloc(&d_FPSetCoordsList_Counts, numQueries * sizeof(unsigned)));
    gpuErrchk(cudaMalloc(&d_L2distances, (MAX_PARENTS_PERQUERY * numQueries) *
                                             sizeof(float)));  // Dim: [numIterations * numQueries]
    gpuErrchk(
        cudaMalloc(&d_L2ParentIds, (MAX_PARENTS_PERQUERY * numQueries) *
                                       sizeof(unsigned)));  // Dim: [numIterations * numQueries]
    gpuErrchk(
        cudaMalloc(&d_nearestNeighbours,
                   (recall_at * numQueries) * sizeof(unsigned)));  // Dim: [recall_at * numQueries]
    gpuErrchk(cudaMalloc(&d_numQueries, sizeof(unsigned)));

    gpuErrchk(cudaMalloc(
        &d_L2distances_aux,
        (MAX_PARENTS_PERQUERY * numQueries) * sizeof(float)));  // Dim: [numIterations * numQueries]
    gpuErrchk(
        cudaMalloc(&d_L2ParentIds_aux, (MAX_PARENTS_PERQUERY * numQueries) *
                                           sizeof(unsigned)));  // Dim: [numIterations * numQueries]
    ...
    // streams Hence creating separate streams for transfers and computations to achieve overlap
    ...
    // There are 3 stages for free'ing: 1) After transferring to Device (i.e. before search) 2)
    // After the iterations and 3) Before termination
#ifdef FREE_AFTERUSE
    // ToDo : To reduce CPU peak memory, the compressed vectors cna be transferred to GPU first and
    // free'd. Then load the graph on CPU
    ...
    do  // this is just to run the entire search multiple runs for consistent stats reporting
    {
        ...
        // Note: parent for row 0, should be medoidID, but initializing entirely to medoidID just
        // hurt
        gpuErrchk(cudaMemcpy(d_L2ParentIds, L2ParentIds, sizeof(unsigned) * numQueries,
                             cudaMemcpyHostToDevice));
        gpuErrchk(cudaMemcpy(d_FPSetCoordsList_Counts, FPSetCoordsList_Counts,
                             sizeof(unsigned) * numQueries, cudaMemcpyHostToDevice));
        ...
        if (bCacheWarmUp) {
            ...
            gpuErrchk(cudaMemcpy(d_neighbors_warmup, neighbors_warmup,
                                 sizeof(unsigned) * uNeighbours_size, cudaMemcpyHostToDevice));
            compute_neighborDist_par_cachewarmup<<<uNeighbours_size, uNeighbours_size / R>>>(
                d_neighbors_warmup, d_compressedVectors);
        }
        ...
        gpuErrchk(cudaMemcpy(d_queriesFP, queriesFP, sizeof(datatype_t) * (D * numQueries),
                             cudaMemcpyHostToDevice));
         ...
         * One thread block is assigned to a query, i.e., 256 threads perform the computation for a
         * query. The block size has been tuned for performance.
         */
        // ToDo: 256 or R
        neighbor_filtering_new<<<numQueries, numThreads_K5, 0, streamKernels>>>(
            d_neighbors, d_neighbors_temp, d_numNeighbors_query, d_numNeighbors_query_temp,
            d_processed_bit_vec, d_parents, d_pIndex, iter, d_nextIter);
        gputimer.Stop();
        ...
        gpuErrchk(cudaMemcpy(bang_counter_tmp, d_numNeighbors_query, sizeof(unsigned) * numQueries,
        ...
        /** [5] Launching the kernel with "numQueries" number of thread-blocks and user specified
         * "numThreads_K2" block size. One thread block is assigned to a query, i.e.,
         * "numThreads_K2" threads perform the computation for a query.
         */

        compute_neighborDist_par<<<numQueries, numThreads_K2, 0, streamKernels>>>(
            d_neighbors, d_numNeighbors_query, d_neighborsDist_query, d_queriesFP, d_pIndex);
         ...
         * One thread block is assigned to a query, i.e., (R+1) threads perform the computation for
         * a query. The kernel  sorts an array of size (R+1) per query, so we do not require (R+1)
         * threads per query.
         */
        compute_BestLSets_par_sort_msort<<<numQueries, numThreads_K3, 0, streamKernels>>>(
            d_neighbors, d_neighbors_aux, d_numNeighbors_query, d_neighborsDist_query,
            d_neighborsDist_query_aux, d_nextIter);

        /** [9] Launching the kernel with "numQueries" number of thread-blocks and (2*L) block size.
         * One thread block is assigned to a query, i.e., (2*L) threads perform the computation for
         * a query. The kernel merges, for every query, two arrays each of whose sizes are
         * upperbounded by L, so we do not require more than 2*L threads per query.
         */

        compute_BestLSets_par_merge<<<numQueries, numThreads_K3_merge, 0, streamKernels>>>(
            d_neighbors, d_numNeighbors_query, d_neighborsDist_query, d_BestLSets, d_BestLSetsDist,
            d_BestLSets_visited, d_parents, iter, d_nextIter, d_BestLSets_count, d_L2ParentIds,
            d_FPSetCoordsList_Counts, d_numQueries);
#else

        compute_BestLSets_par_sort_msort_new<<<numQueries, max(numThreads_K3, numThreads_K3_merge),
                                               0, streamKernels>>>(
            d_neighbors,
            // d_neighbors_aux,
            d_numNeighbors_query, d_neighborsDist_query,
            // d_neighborsDist_query_aux,
            d_BestLSets, d_BestLSetsDist, d_BestLSets_visited, d_parents, iter, d_nextIter,
            d_BestLSets_count, d_L2ParentIds, d_FPSetCoordsList_Counts, d_numQueries);
        ...
        do {
            ...
            ++iter;
            gpuErrchk(cudaMemset(d_numNeighbors_query, 0, sizeof(unsigned) * numQueries));
            /** [11] Launching the kernel with "numQueries" number of thread-blocks and block size
             * of 256 One thread block is assigned to a query, i.e., 256 threads perform the
             * computation for a query. The block size has been tuned for performance.
             */
            neighbor_filtering_new<<<numQueries, numThreads_K5, 0, streamKernels>>>(
                d_neighbors, d_neighbors_temp, d_numNeighbors_query, d_numNeighbors_query_temp,
                d_processed_bit_vec, d_parents, d_pIndex, iter, d_nextIter);
            ...
            gpuErrchk(cudaMemcpy(bang_counter_tmp, d_numNeighbors_query,
                                 sizeof(unsigned) * numQueries, cudaMemcpyDeviceToHost));
            for (int i = 0; i < numQueries; i++) {
                bang_counter += bang_counter_tmp[i];
            }
...
#ifdef OLD_MERGE
            /** [8] Launching the kernel with "numQueries" number of thread-blocks and (R+1) block
             * size. One thread block is assigned to a query, i.e., (R+1) threads perform the
             * computation for a query. The kernel  sorts an array of size (R+1) per query, so we do
             * not require (R+1) threads per query.
             */
            compute_BestLSets_par_sort_msort<<<numQueries, numThreads_K3, 0, streamKernels>>>(
                d_neighbors, d_neighbors_aux, d_numNeighbors_query, d_neighborsDist_query,
                d_neighborsDist_query_aux, d_nextIter);

            /** [9] Launching the kernel with "numQueries" number of thread-blocks and (2*L) block
             * size. One thread block is assigned to a query, i.e., (2*L) threads perform the
             * computation for a query. The kernel merges, for every query, two arrays each of whose
             * sizes are upperbounded by L, so we do not require more than 2*L threads per query.
             */
            compute_BestLSets_par_merge<<<numQueries, numThreads_K3_merge, 0, streamKernels>>>(
                d_neighbors, d_numNeighbors_query, d_neighborsDist_query, d_BestLSets,
                d_BestLSetsDist, d_BestLSets_visited, d_parents, iter, d_nextIter,
                d_BestLSets_count, d_L2ParentIds, d_FPSetCoordsList_Counts, d_numQueries);
#else
            compute_BestLSets_par_sort_msort_new<<<
                numQueries, max(numThreads_K3, numThreads_K3_merge), 0, streamKernels>>>(
                d_neighbors,
                // d_neighbors_aux,
                d_numNeighbors_query, d_neighborsDist_query,
                // d_neighborsDist_query_aux,
            ...
            gpuErrchk(cudaMemcpy(
                &nextIter, d_nextIter, sizeof(bool),
                cudaMemcpyDeviceToHost));  // d_nextIter calculated in compute_parent<<< >>>
            // Note: Default Stream operations (cmputation or memory transfers) cannot overlap with
            // operatiosn on other sterrams. Hence, the above call could act as a synchronization
            // mechanism to ensure all kernels are done (next parent ready) before we start seeking
            // neighbours on CPU
        ...
        compute_L2Dist<<<numQueries, K4_blockSize>>>(d_FPSetCoordsList, d_FPSetCoordsList_Counts,
                                                     d_queriesFP, d_L2ParentIds, d_L2distances,
                                                     d_nearestNeighbours, d_numQueries, d_pIndex);

#ifdef _DBG1
        cudaDeviceSynchronize();
        unsigned* pTempDists =
            (unsigned*)malloc(sizeof(unsigned) * (MAX_PARENTS_PERQUERY * numQueries));
        gpuErrchk(cudaMemcpy(pTempDists, d_L2distances,
                             sizeof(unsigned) * (MAX_PARENTS_PERQUERY * numQueries),
                             cudaMemcpyDeviceToHost));

        unsigned* pTempParents =
            (unsigned*)malloc(sizeof(unsigned) * (MAX_PARENTS_PERQUERY * numQueries));
        gpuErrchk(cudaMemcpy(pTempParents, d_L2ParentIds,
                             sizeof(unsigned) * (MAX_PARENTS_PERQUERY * numQueries),
                             cudaMemcpyDeviceToHost));

        unsigned* pTempNumParents = (unsigned*)malloc(sizeof(unsigned) * numQueries);
        gpuErrchk(cudaMemcpy(pTempNumParents, d_FPSetCoordsList_Counts,
                             sizeof(unsigned) * numQueries, cudaMemcpyDeviceToHost));

        // print parentIDs and distances for Query 0

        for (int i = 0; i < pTempNumParents[nQueryID]; i++) {
            printf("Parent = %d \t distance = %d\n", pTempParents[(numQueries * i) + nQueryID],
                   pTempDists[(numQueries * i) + nQueryID]);
        }

#endif
#endif
        compute_NearestNeighbours<<<numQueries, MAX_PARENTS_PERQUERY>>>(
            d_L2ParentIds, d_L2ParentIds_aux, d_FPSetCoordsList_Counts, d_L2distances,
            d_L2distances_aux, d_nearestNeighbours, d_numQueries, d_recall, d_BestLSets);
        ...
        start = std::chrono::high_resolution_clock::now();
        gpuErrchk(cudaMemcpy(nearestNeighbours, d_nearestNeighbours,
                             sizeof(unsigned) * (recall_at * numQueries), cudaMemcpyDeviceToHost));
        stop = std::chrono::high_resolution_clock::now();
        time_transfer +=
            std::chrono::duration_cast<std::chrono::nanoseconds>(stop - start).count() / 1000.0;
...
#pragma omp critical
        ...
        for (unsigned idx = 1; idx < time_B1_vec.size(); ++idx) {
            time_B1_avg =
                time_B1_avg + (time_B1_vec[idx] - time_B1_avg) / (idx + 1);  // running average
        ...
        for (unsigned idx = 1; idx < time_B2_vec.size(); ++idx) {
            time_B2_avg =
                time_B2_avg + (time_B2_vec[idx] - time_B2_avg) / (idx + 1);  // running average
        ...
        // Like DiskANN o/p we wannted to run with multiple L's in a single invocation of the
        ...
        gpuErrchk(cudaMemcpy(FPSetCoordsList_Counts_tmp, d_FPSetCoordsList_Counts,
                             sizeof(unsigned) * numQueries, cudaMemcpyDeviceToHost));
        for (int i = 0; i < numQueries; i++) {
    ...
    }  // outer do-while for running the search multiple times. The stats across runs can be
       // averaged etc
    // for a reporting results in a consistent manner (i.e. avoid affect of outliers)
    while (1);
}

/** This kernel filters out those nodes from the neighborhood of nodes in "d_parents" that  have
 * already been processed before (in a previous iteration) and populates d_neighbors with only the
 * new unseen ones.
 * @param d_neighbors This is populated by the kernel to store the the node ids that will be
 * considered in the next iteration, for each query.
 * @param d_neighbors_temp This stores the node ids of the neighborhood of the nodes fetched from
 * host, for every query.
 * @param d_numNeighbors_query This is populated by the kernel to store the number of nodes in
 * d_neighbors, for every query.
 * @param d_numNeighbors_query_temp This stores the number of neighbors fetched from the host, for
 * every query.
 * @param d_processed_bit_vec This is a boolean vector for keeping track of whether a node is
 * processed on not.
 * @param beamWidth This is the beamwidth.
 */

__global__ void neighbor_filtering_new(unsigned* d_neighbors, unsigned* d_neighbors_temp,
                                       unsigned* d_numNeighbors_query,
                                       unsigned* d_numNeighbors_query_temp,
                                       bool* d_processed_bit_vec, unsigned* d_parents,
                                       uint8_t* d_pIndex, unsigned iter, bool* d_nextIter) {
    ...
    *d_nextIter = false;
    ...
    unsigned offset_bit_vec =
        queryID * BF_MEMORY;  // Offset into d_processed_bit_vec vector of bloom filter
    bool*              d_processed_bit_vec_start = d_processed_bit_vec + offset_bit_vec;
    unsigned long long parentID;

    if (iter == 1) {
        // If this is first iteration, set the bits corresponding to MEDOID so that its not taken as
    ...
    unsigned* bound = (unsigned*)(d_pIndex + ((unsigned long long)INDEX_ENTRY_LEN * parentID) +
                                  D * sizeof(datatype_t));
    // For each neighbor of current parent in d_graph array check if its corresponding bits in the
    // d_processed_bit_vec are already set
 ...
 * THe main intent to is to access the compressed vectors of the neighbour and not the distance
 calculation (which is dummy operation) as such ^ One thread is responsible for distance calcuation
 of one node
 */
...
__global__ void compute_neighborDist_par_cachewarmup(unsigned* d_neighbors,
    ...
    for (unsigned j = tid; j < numNeighbors;
         j += (blockDim.x)) {  // assign eight threads to a neighbor, within a query

        unsigned long long compressed_vector_offset =
            ((unsigned long long)d_neighbors_start[j]) * CHUNKS;
...
/**
 * This kernel computes for every query, the distance between the neighbors and the query through
 * lookups to the PQ Dist Table and the compressed vectors of the neighbors.
 * @param d_neighbors This is the concatenated list of node ids for all queries whose distances have
 * to be computed with the respective queries.
 ...
 * @param d_neighborsDist_query This is populated by the kernel with the distances between the node
 * ids in d_neighbors and the respective query for all queries.
 * @param beamWidth This is the beamwidth.
 */

__global__ void compute_neighborDist_par(unsigned* d_neighbors, unsigned* d_numNeighbors_query,
                                         float* d_neighborsDist_query, datatype_t* d_queriesFP,
                                         uint8_t* d_pIndex) {
    unsigned tid     = threadIdx.x;
    unsigned queryID = blockIdx.x;
...
#define THREADS_PER_NEIGHBOR 8
    ...
    for (unsigned j = tid / THREADS_PER_NEIGHBOR; j < numNeighbors;
         j += (blockDim.x) /
              THREADS_PER_NEIGHBOR) {  // assign eight threads to a neighbor, within a query
        ...
        for (unsigned i = tid % THREADS_PER_NEIGHBOR; i < D; i += THREADS_PER_NEIGHBOR) {
            float diff = (float)(*(pBase + i)) -
                         (float)d_queriesFP_start[i];  // d_quriesFP must contain float
...
__global__ void compute_L2Dist(datatype_t* d_FPSetCoordsList, unsigned* d_FPSetCoordsList_Counts,
                               datatype_t* d_queriesFP, unsigned* d_L2ParentIds,
                               float* d_L2distances, unsigned* d_nearestNeighbours,
                               unsigned* d_numQueries, uint8_t* d_pIndex) {
    ...
    for (unsigned ii = tid; ii < numNodes; ii += blockDim.x) {
        datatype_t* pBase =
            (datatype_t*)(d_pIndex + (unsigned long long)d_L2ParentIds[numQueries * ii + queryID] *
                                         INDEX_ENTRY_LEN);
        float L2Dist = 0.0;
...
__global__ void compute_NearestNeighbours(unsigned* d_L2ParentIds, unsigned* d_L2ParentIds_aux,
                                          unsigned* d_FPSetCoordsList_Counts, float* d_L2distances,
                                          float* d_L2distances_aux, unsigned* d_nearestNeighbours,
                                          unsigned* d_numQueries, unsigned* d_recall,
    ...
    if (tid >= numNeighbors || numNeighbors <= 0)
        return;
    ...
    for (unsigned subArraySize = 2; subArraySize < 2 * numNeighbors; subArraySize *= 2) {
        ...
        if (tid >= start && tid < mid) {
            unsigned lowerBound = lower_bound_d_ex(
                &d_L2distances[mid * (*d_numQueries)], 0, end - mid,
                d_L2distances[(tid * (*d_numQueries)) + queryID], *d_numQueries, queryID);
            shm_pos[tid] = lowerBound + tid;  // Position for this element
        }

        if (tid >= mid && tid < end) {
            unsigned upperBound = upper_bound_d_ex(
                &d_L2distances[start * (*d_numQueries)], 0, mid - start,
                d_L2distances[(tid * (*d_numQueries)) + queryID], *d_numQueries, queryID);
        ...
        for (int i = tid; i < numNeighbors; i += blockDim.x) {
            d_L2distances_aux[(shm_pos[i] * (*d_numQueries)) + queryID] =
                d_L2distances[(i * (*d_numQueries)) + queryID];
            d_L2ParentIds_aux[(shm_pos[i] * (*d_numQueries)) + queryID] =
                d_L2ParentIds[(i * (*d_numQueries)) + queryID];
        }
        __syncthreads();
        // Copy the auxiliary array to original array
        for (int i = tid; i < numNeighbors; i += blockDim.x) {
            d_L2distances[(i * (*d_numQueries)) + queryID] =
                d_L2distances_aux[(i * (*d_numQueries)) + queryID];
            d_L2ParentIds[(i * (*d_numQueries)) + queryID] =
                d_L2ParentIds_aux[(i * (*d_numQueries)) + queryID];
        }
        __syncthreads();
    }
#endif
...
/** This kernel sorts the neighbors (for every query) in increasing order of their distances to the
 * query, using parallel merge sort The sorted list of neighbors is stored in "d_neighbors" at the
 * end of the kernel.
 * @param d_neighbors This is the concatenated list of node ids for all queries whose distances have
 * to be computed with the respective queries.
 * @param d_neighbors_aux This is an auxiliary array for d_neighbors, used for merge sort.
 * @param d_numNeighbors_query This stores for every query the number of nodes in d_neighbors.
 * @param d_neighborsDist_query This contains the distances between the node ids in d_neighbors and
 * the respective query for all queries.
 * @param d_neighborsDist_query_aux This is an auxiliary array for d_neighborsDist_query, used for
 * merge sort.
 * @param d_nextIter This maintains boolean flags for all queries, to decide if a query is to
 * processed in the next iteration.
 * @param beamWidth This is the beamwidth.
 */

#ifdef OLD_MERGE
__global__ void compute_BestLSets_par_sort_msort(unsigned* d_neighbors, unsigned* d_neighbors_aux1,
                                                 unsigned* d_numNeighbors_query,
    ...
    *d_nextIter           = false;
    ...
    unsigned            offset =
        queryID * (R + 1);  // Offset into d_neighborsDist_query, d_neighborsDist_query_aux,
                            // d_neighbors_aux and d_neighbors arrays
    ...
    for (unsigned subArraySize = 2; subArraySize < 2 * numNeighbors; subArraySize *= 2) {
        ...
        if (tid >= start && tid < mid) {
            unsigned lowerBound = lower_bound_d(&d_neighborsDist_query[offset + mid], 0, end - mid,
                                                d_neighborsDist_query[offset + tid]);
            shm_pos[tid]        = lowerBound + tid;  // Position for this element
        }

        if (tid >= mid && tid < end) {
            unsigned upperBound = upper_bound_d(&d_neighborsDist_query[offset + start], 0,
                                                mid - start, d_neighborsDist_query[offset + tid]);
            shm_pos[tid]        = start + (upperBound + tid - mid);  // Position for this element
        }
        __syncthreads();
        __threadfence_block();
...
/** This kernel merges the current Best_L_Set (candidate set) with the list of sorted neighbors (in
 d_neighbors) to update the candidate set.
 * @param d_neighbors This is the concatenated list of node ids for all queries whose distances have
 to be computed with the respective queries.
 * @param d_numNeighbors_query This stores for every query the number of nodes in d_neighbors.
 * @param d_neighborsDist_query This contains the distances between the node ids in d_neighbors and
 ...
 * @param d_BestLSetsDist contains the contatenated list of the distances of the candidate nodes to
 the respective query, for all queries.
 * @param d_BestLSets_visited This maintains the boolean information about if an entry in the
 d_BestLSets has been processed or not.
 * @param d_parents This is populated with the list of nodes whose neighborhood information has to
 be fetched in the next iteration of the outer while loop, for all queries.
 * @param beamWidth This is the beamwidth.
 * @param first This is a boolean flag to differentiate between the first call to kernel (outside
 the while loop) from the rest of the calls.
 * @param d_nextIter This maintains boolean flags for all queries, to decide if a query is to
 processed in the next iteration.
 * @param d_BestLSets_count contains the number of nodes in the d_BestLSets, for every query.
 */

__global__ void compute_BestLSets_par_merge(unsigned* d_neighbors, unsigned* d_numNeighbors_query,
                                            float* d_neighborsDist_query, unsigned* d_BestLSets,
                                            float* d_BestLSetsDist, bool* d_BestLSets_visited,
                                            unsigned* d_parents, unsigned iter, bool* d_nextIter,
                                            unsigned* d_BestLSets_count, unsigned* d_L2ParentIds,
    ...
    if (numNeighbors >
        0) {  // If the number of neighbors after filteration is zero then no sense of merging

        if (iter == 1) {  // If this is the first call to compute_BestLSets_par_merge by this query
                          // then initialize d_BestLSets, d_BestLSetsDist...
            ...
            // if both Best_L_Set_size and numNeighbors is less than L, then the max of the two will
            // be the newBest_L_Set_size otherwise it will be L
            ...
            __syncthreads();
            if (tid < nbrsBound) {
                shm_pos[tid] = lower_bound_d(shm_currBestLSetsDist, 0, Best_L_Set_size,
                                             shm_neighborsDist_query[tid]) +
                               tid;
            }
            if (tid >= nbrsBound && tid < (nbrsBound + Best_L_Set_size)) {
                shm_pos[tid] = upper_bound_d(shm_neighborsDist_query, 0, nbrsBound,
                                             shm_currBestLSetsDist[tid - nbrsBound]) +
                               (tid - nbrsBound);
            }
            ...
            if (tid >= nbrsBound && tid < (nbrsBound + Best_L_Set_size) &&
                shm_pos[tid] < newBest_L_Set_size) {
                ...
                shm_BestLSets_visited[shm_pos[tid]] =
                    d_BestLSets_visited[queryID * L + (tid - nbrsBound)];
            }
            __syncthreads();
            __threadfence_block();
    ...
    if (tid == 0) {
        ...
        if (parentIndex != 0)  // parentIndex == 0 is the termination condition for the algorithm.
        {
            *d_nextIter = true;
            ...
            d_L2ParentIds[(iter * numQueries) + queryID] =
                d_parents[queryID * (SIZEPARENTLIST) + parentIndex];
        }
    }
}
...
__global__ void compute_BestLSets_par_sort_msort_new(
    unsigned* d_neighbors,
    // unsigned* d_neighbors_aux,
    unsigned* d_numNeighbors_query, float* d_neighborsDist_query,
    // float* d_neighborsDist_query_aux,
    unsigned* d_BestLSets, float* d_BestLSetsDist, bool* d_BestLSets_visited, unsigned* d_parents,
    unsigned iter, bool* d_nextIter, unsigned* d_BestLSets_count, unsigned* d_L2ParentIds,
    unsigned* d_FPSetCoordsList_Counts, unsigned* d_numQueries) {
    unsigned tid          = threadIdx.x;
    unsigned queryID      = blockIdx.x;
    unsigned numNeighbors = d_numNeighbors_query[queryID];
    *d_nextIter           = false;
    ...
    __shared__ unsigned shm_pos[R + 1];
    unsigned            offset =
        queryID * (R + 1);  // Offset into d_neighborsDist_query, shm_neighborsDist_query_aux,
                            // d_neighbors_aux and d_neighbors arrays
    ...
    if (numNeighbors >
        ...
        if (iter == 1) {  // If this is the first call to compute_BestLSets_par_merge by this query
            ...
            __syncthreads();
            if (tid < nbrsBound) {
                shm_pos1[tid] = lower_bound_d(shm_currBestLSetsDist, 0, Best_L_Set_size,
                                              shm_neighborsDist_query_aux[tid]) +
                                tid;
            }
            if (tid >= nbrsBound && tid < (nbrsBound + Best_L_Set_size)) {
                shm_pos1[tid] = upper_bound_d(shm_neighborsDist_query_aux, 0, nbrsBound,
                                              shm_currBestLSetsDist[tid - nbrsBound]) +
                                (tid - nbrsBound);
            }
            ...
            if (tid >= nbrsBound && tid < (Temp) && shm_pos1[tid] < newBest_L_Set_size) {
                ...
                shm_BestLSets_visited[shm_pos1[tid]] =
                    d_BestLSets_visited[queryID * L + (tid - nbrsBound)];
            }
            __syncthreads();
            __threadfence_block();
...
/*Helper device function which returns the position first element in the range [lo,hi), which has a
 * value not less than 'target'.*/
...
/*Helper device function which returns the position first element in the range [lo,hi), which has a
 * value not less than 'target'.*/
__device__ unsigned lower_bound_d_ex(float arr[], unsigned lo, unsigned hi, float target,
                                     unsigned row_size, unsigned queryID) {
    unsigned mid;
...
/*Helper device function which returns the position first element in the range (lo,hi], which has a
 * value greater than 'target'.*/
__device__ unsigned upper_bound_d_ex(float arr[], unsigned lo, unsigned hi, float target,
                                     unsigned row_size, unsigned queryID) {
    unsigned mid;
...
NeighbourList GetNeighbours(uint8_t* pGraph, unsigned curreParent, NodeIDMap& mapNodeIDToNode) {
...
void bfs(unsigned uMedoid, const unsigned nNodesToDiscover, unsigned& visit_counter,
         NodeIDMap& mapNodeIDToNode, uint8_t* pGraph) {
    Node* pNode     = (Node*)malloc(sizeof(Node));
