...
#define MEDOID 123                  // 221898 // sift 1B
#define N 10000                     // total number of nodes in the dataset
#define NUMTHREADS_COMPUTEPARENT 1  // surprisingly, found 1 to work better than 100!
#endif
...
__global__ void populate_pqDist_par(float* d_pqTable, float* d_pqDistTables,
                                    datatype_t* d_queriesFP, unsigned* d_chunksOffset,
                                    float* d_centroid, unsigned n_chunks);
...
__global__ void compute_parent1(unsigned* d_neighbors, unsigned* d_numNeighbors_query,
                                ...
                                unsigned* d_mark, unsigned* d_iter, unsigned* d_L2ParentIds,
                                unsigned* d_FPSetCoordsList_Counts, unsigned* d_numQueries);

__global__ void compute_parent2(unsigned* d_neighbors, unsigned* d_numNeighbors_query,
                                float* d_neighborsDist_query, unsigned* d_BestLSets,
                                float* d_BestLSetsDist, bool* d_BestLSets_visited,
                                unsigned* d_parents, bool* d_nextIter, unsigned* d_BestLSets_count,
                                unsigned* d_mark, unsigned* d_iter, unsigned* d_L2ParentIds,
...
__global__ void compute_BestLSets_par_sort_msort(unsigned* d_neighbors, unsigned* d_neighbors_aux,
...
class ANNException {
   public:
    ...
    ANNException(const std::string& message, int errorCode, const std::string& funcSig,
...
inline void load_truthset(const std::string& bin_file, uint32_t*& ids, float*& dists, size_t& npts,
                          size_t& dim) {
...
template <typename T>
inline void load_aligned_bin_impl(std::basic_istream<char>& reader, size_t actual_file_size,
                                  T*& data, size_t& npts, size_t& dim, size_t& rounded_dim) {
    int npts_i32, dim_i32;
    reader.read((char*)&npts_i32, sizeof(int));
    reader.read((char*)&dim_i32, sizeof(int));
    npts = (unsigned)npts_i32;
...
template <typename T>
inline void load_aligned_bin(const std::string& bin_file, T*& data, size_t& npts, size_t& dim,
                             size_t& rounded_dim) {
...
template <typename T>
inline void load_bin_impl(std::basic_istream<char>& reader, size_t actual_file_size, T*& data,
