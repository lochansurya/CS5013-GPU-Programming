...
/**
 * @file engine_vis.cu
 * @brief Visualization-specific engine implementation.
 * 
 * This version of the engine is optimized for visualization, supporting
 * step-by-step snapshotting and detailed query info logging.
 */
...
public:
    ...
    BenchmarkResult process_workload(int batch_size, const char* output_dir, int beam_width) {
        ...
        for (size_t i = 0; i < ops.size(); i += batch_size) {
            ...
            // Snapshot for Toy 2D
            if (DIM_SZ == 2) {
                const QueryInfo* qi = (!searches.empty() && !query_stats.empty()) ? &query_stats.back() : nullptr;
                save_snapshot(batch_idx, output_dir, qi);
            }
    ...
    void save_snapshot(int step, const char* output_dir, const QueryInfo* q_info = nullptr) {
        ...
        // Write Search Info
        int has_search = (q_info != nullptr) ? 1 : 0;
        fwrite(&has_search, sizeof(int), 1, f);
        
        if (has_search) {
            fwrite(q_info->query_vec.data(), sizeof(float), DIM_SZ, f);
            
            int found_count = q_info->found_ids.size();
            fwrite(&found_count, sizeof(int), 1, f);
            fwrite(q_info->found_ids.data(), sizeof(int), found_count, f);
            
            int gt_count = q_info->gt_ids.size();
            fwrite(&gt_count, sizeof(int), 1, f);
            fwrite(q_info->gt_ids.data(), sizeof(int), gt_count, f);
        }
...
int main(int argc, char** argv) {
    ...
    // Always Toy Mode for this file
    VamanaEngine<2, 1000> engine;

    // Single Pass Visualization Run
    engine.load_data(graph_file, workload_file);
    engine.reset_gpu_state();
    
    // Use beam_width=32 (or could be arg)
    engine.process_workload(batch_size, output_dir, 32);
