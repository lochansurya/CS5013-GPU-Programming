...
__global__ void getNeighbors(uint8_t* d_graph, unsigned batchStart, unsigned* d_neighbors,
    ...
    unsigned* degreePtr =
        (unsigned*)(d_graph + extendedQueryID * graphEntrySize + D * sizeof(float));
...
__global__ void mergeIntoVisitedSet(unsigned* d_visitedSetCount, unsigned* d_visitedSet,
                                    float* d_visitedSetDists, unsigned* d_neighborsCount,
    ...
    if (tid < visitedSetSize) {
        ...
        unsigned before = upperBound(&d_visitedSetDists[visitedSetOffset], 0, visitedSetSize,
...
// Robust Prune
__global__ void pruneOutNeighbors(uint8_t* d_graph, unsigned batchStart, unsigned* d_visitedSet,
                                  unsigned* d_visitedSetCount, float* d_visitedSetDists,
                                  NodeState* d_visitedSetStatus,

                                  float* d_queryVecs, uint8_t* d_reverseEdgeIndex, float alpha) {
        ...
        if (tid == 0) {
            for (unsigned i = 0; i < numNodes; i++) {
                ...
                *pStarShared = d_visitedSet[visitedSetOffset + i];
                ...
                unsigned* entryPtr =
                    (unsigned*)&d_reverseEdgeIndex[*pStarShared * reverseIndexEntrySize];
...
void computeOutNeighbors(uint8_t* d_graph, float* d_queryVecs, unsigned* d_visitedSets,
                         unsigned* d_visitedSetCount, float alpha, uint8_t* d_reverseEdgeIndex,
                         unsigned batchStart, unsigned batchSize) {
    bool log = false;
    ...
    gputimer.Start();
    getNeighbors<<<batchSize, R>>>(d_graph, batchStart, d_neighbors, d_neighborsCount);
    gputimer.Stop();
    gpuErrchk(cudaDeviceSynchronize());
    // printf("getNeighbors GPU time: %f ms\n", gputimer.Elapsed());

    gputimer.Start();
    computeDists<<<batchSize, R * 8>>>(d_graph, d_neighbors, d_neighborsCount, d_queryVecs,
                                       d_neighborsDists, (R + 1));
    gputimer.Stop();
    // gpuErrchk(cudaDeviceSynchronize());
    // printf("computeDists GPU time: %f ms\n", gputimer.Elapsed());

    gputimer.Start();
    sortByDistance<<<batchSize, R + 1, (R + 1) * sizeof(unsigned)>>>(
        d_neighbors, d_neighborsCount, d_neighborsDists, d_neighborsAux, d_neighborsDistsAux,
        R + 1);
    gputimer.Stop();
    // gpuErrchk(cudaDeviceSynchronize());
    ...
    gputimer.Start();
    computeDists<<<batchSize, 1024>>>(d_graph, d_visitedSets, d_visitedSetCount, d_queryVecs,
                                      d_visitedSetDists, MAX_PARENTS_PERQUERY);
    gputimer.Stop();
    // gpuErrchk(cudaDeviceSynchronize());
    // printf("computeDists GPU time: %f ms\n", gputimer.Elapsed());
    ...
    sortByDistance<<<batchSize, MAX_PARENTS_PERQUERY, MAX_PARENTS_PERQUERY * sizeof(unsigned)>>>(
        d_visitedSets, d_visitedSetCount, d_visitedSetDists, d_visitedSetAux, d_visitedSetDistsAux,
    ...
    gputimer.Start();
    mergeIntoVisitedSet<<<batchSize, MAX_PARENTS_PERQUERY + R>>>(
        d_visitedSetCount, d_visitedSets, d_visitedSetDists, d_neighborsCount, d_neighbors,
        d_neighborsDists);
    gputimer.Stop();
    // gpuErrchk(cudaDeviceSynchronize());
    ...
    gputimer.Start();
    pruneOutNeighbors<<<batchSize, 32>>>(d_graph, batchStart, d_visitedSets, d_visitedSetCount,
                                         d_visitedSetDists, d_visitedSetStatus,

                                         d_queryVecs, d_reverseEdgeIndex, alpha);
    gputimer.Stop();
    // gpuErrchk(cudaDeviceSynchronize());
