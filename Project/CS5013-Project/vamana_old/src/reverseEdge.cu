...
__global__ void parseReverseIndex(uint8_t* d_reverseEdgeIndex, unsigned* d_reverseEdges,
                                  unsigned* d_reverseEdgeCount, unsigned* degreeCounts) {
    unsigned queryID = blockIdx.x;
    unsigned tid     = threadIdx.x;
...
__global__ void getPrunableQueryIDs(unsigned* d_reverseEdgeCount, unsigned* d_queryIDs,
...
__global__ void mergeIntoReverseEdges(unsigned* d_reverseEdgeCount, unsigned* d_reverseEdges,
                                      float* d_reverseEdgeDists, unsigned* d_neighborsCount,
                                      unsigned* d_neighbors, float* d_neighborsDist) {
    unsigned queryID = blockIdx.x;
    unsigned tid     = threadIdx.x;
    ...
    if (tid < reverseEdgeCount) {
        ...
        unsigned before = upperBound(&d_reverseEdgeDists[reverseEdgeOffset], 0, reverseEdgeCount,
...
__global__ void pruneReverseEdges(uint8_t* d_graph, unsigned* d_queryIDs, unsigned* d_reverseEdges,
                                  unsigned* d_reverseEdgeCount, float* d_reverseEdgeDists,
                                  NodeState* d_reverseEdgeStatus,

                                  float* d_queryVecs, float alpha) {
...
void computeReverseEdges(uint8_t* d_graph, uint8_t* d_reverseEdgeIndex, float alpha) {
    ...
    parseReverseIndex<<<N, 1024>>>(d_reverseEdgeIndex, d_reverseEdges, d_reverseEdgeCount,
                                   degreeSum);
    ...
    getPrunableQueryIDs<<<(N + numThreads - 1) / numThreads, numThreads>>>(
    ...
    computeDists<<<N, 1024>>>(d_graph, d_reverseEdges, d_reverseEdgeCount, d_queryVecs,
                              d_reverseEdgeDists, MAX_REVERSE_INDEX_ENTRIES);

    // sortByDistance<<<N, MAX_REVERSE_INDEX_ENTRIES,
    sortByDistance<<<N, 1024, MAX_REVERSE_INDEX_ENTRIES * sizeof(unsigned)>>>(
        d_reverseEdges, d_reverseEdgeCount, d_reverseEdgeDists, d_reverseEdgesAux,
        d_reverseEdgeDistsAux, MAX_REVERSE_INDEX_ENTRIES);

    getNeighbors<<<N, R>>>(d_graph, 0, d_neighbors, d_neighborsCount);

    computeDists<<<N, R * 8>>>(d_graph, d_neighbors, d_neighborsCount, d_queryVecs, d_neighborDists,
                               (R + 1));

    sortByDistance<<<N, R + 1, (R + 1) * sizeof(unsigned)>>>(
        d_neighbors, d_neighborsCount, d_neighborDists, d_neighborsAux, d_neighborDistsAux, R + 1);

    mergeIntoReverseEdges<<<N, 1024>>>(
        d_reverseEdgeCount,
        // mergeIntoReverseEdges<<<N, R+MAX_REVERSE_INDEX_ENTRIES>>>(d_reverseEdgeCount,
        d_reverseEdges, d_reverseEdgeDists, d_neighborsCount, d_neighbors, d_neighborDists);

    // unsigned iter = 0;

    h_queryCount = N;
    pruneReverseEdges<<<h_queryCount, 32>>>(d_graph, d_queryIDs, d_reverseEdges, d_reverseEdgeCount,
                                            d_reverseEdgeDists, d_reverseEdgeStatus,
